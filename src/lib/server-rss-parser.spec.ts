...
describe('server/lib/rss-parser', () => {
  ...
  it('parses atom entry fields and fallback link', () => {
    const source = makeSource();
    const xml = `<?xml version="1.0"?>
  ...
  it('prioritizes media image over thumbnail and picks largest image', () => {
    const source = makeSource();
    const xml = `<?xml version="1.0"?>
      <rss version="2.0" xmlns:media="http://search.yahoo.com/mrss/">
        <channel>
  ...
  it('strips CDATA wrappers and chooses the longest summary candidate', () => {
    const source = makeSource();
    const xml = `<?xml version="1.0"?>
      <rss version="2.0">
        <channel>
          <item>
            ...
            <content:encoded><![CDATA[<p>Resumen largo con mas detalle y contexto.</p><p>Segundo parrafo.</p>]]></content:encoded>
          </item>
        </channel>
      </rss>`;
    ...
    expect(first?.summary).toBe('<p>Resumen largo con mas detalle y contexto.</p><p>Segundo parrafo.</p>');
  });

  it('throws for empty and unsupported formats', () => {
    const source = makeSource();
