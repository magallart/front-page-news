...
@Component({
  selector: 'app-source-section-filters',
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <aside class="rounded-xl border border-border bg-foreground p-4 text-background" aria-label="Filtros por sección">
            ...
            @for (section of sections(); track section) {
              <label class="flex cursor-pointer items-center gap-2 text-sm text-background">
                <input
                  ...
                  (change)="onSectionToggle(section, $event)"
                />
...
export class SourceSectionFiltersComponent {
  readonly sections = input.required<readonly string[]>();
  readonly selectedSections = input.required<readonly string[]>();
  readonly sortDirection = input.required<'asc' | 'desc'>();

  readonly selectedSectionsChange = output<readonly string[]>();
  readonly sortDirectionChange = output<'asc' | 'desc'>();

  protected readonly formatSectionLabel = formatSectionLabel;

  protected isSectionSelected(section: string): boolean {
    return this.selectedSections().includes(section);
  }

  protected onSectionToggle(section: string, event: Event): void {
    const checkbox = event.target as HTMLInputElement | null;
    const checked = checkbox?.checked ?? false;
    const currentSections = this.selectedSections();
    const nextSections = checked
      ? [...new Set([...currentSections, section])]
      : currentSections.filter((item) => item !== section);
    this.selectedSectionsChange.emit(nextSections);
  }
  ...
  protected selectAllSections(): void {
    this.selectedSectionsChange.emit(this.sections());
  }

  protected clearAllSections(): void {
    this.selectedSectionsChange.emit([]);
  }
}
